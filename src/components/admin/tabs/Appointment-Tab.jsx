    ...
    <div className="bg-white rounded-lg shadow overflow-hidden">
              ...
              appointments.map((appointment) => (
                <tr key={appointment.id_janji}>
                  <td className="px-4 md:px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                    {appointment.pasien?.nama || appointment.id_pasien || "-"}
                  </td>
                  ...
                  <td className="px-4 md:px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {["pending", "terpesan"].includes(appointment.status) ? (
                      <div className="flex space-x-2">
                        ...
                        >
                          {updatingId === appointment.id_janji
                            ? "..."
                            : "Konfirmasi"}
                        </button>
                        <button
      ...
      <div className="p-4 border-t flex flex-col sm:flex-row justify-between items-center gap-2">
        <div className="text-sm text-gray-500 order-2 sm:order-1">
          Menampilkan {(meta.currentPage - 1) * meta.itemCount + 1}-{" "}
          {Math.min(meta.currentPage * meta.itemCount, meta.totalItems)} dari{" "}
          {meta.totalItems} janji temu
        </div>
